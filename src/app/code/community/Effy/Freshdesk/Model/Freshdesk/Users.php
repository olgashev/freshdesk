...
/**
 * Class Effy_Freshdesk_Model_Freshdesk_Users
 *
 */
class Effy_Freshdesk_Model_Freshdesk_Users extends Effy_Freshdesk_Model_Freshdesk_Abstract
{
    const CACHE_ID_SUFFIX_USER          = 'user';
    const CACHE_ID_SUFFIX_USER_BY_EMAIL = 'user_email';
    const CACHE_ID_SUFFIX_AGENTS          = 'agents';

    const USER = 'user';

    const URL_CONTACT       = 'api/v2/contacts/%d';
    const URL_CONTACTS      = 'api/v2/contacts';
    const URL_AGENTS      = 'api/v2/agents';
    ...
    public function getUsers() {
        $this->setUrlSuffix(self::URL_AGENTS);
        if (Mage::app()->useCache(self::CACHE_TYPE)) {
            $usersJson = Mage::app()->loadCache($this->getCacheId() . '_' . self::CACHE_ID_SUFFIX_AGENTS);
            if(!$usersJson) {
                ...
                $usersJson = trim($response->getRawBody());
                if (empty($usersJson)) { 
                    return array();
                }
                Mage::app()->saveCache($usersJson, $this->getCacheId() . '_' . self::CACHE_ID_SUFFIX_AGENTS, $this->getCacheTags(), Mage::getStoreConfig('effy_freshdesk/cache/expiry_lifetime'));
            }
        }   
        else {
            $response = $this->request(Zend_Http_Client::GET);
        ...
        $usersFeed = Zend_Json::decode($usersJson);
        $users = array('' => 'No Agent');
        foreach ($usersFeed as $key => $user) {
            $users[$user['id']] = $user['contact']['name'];            
        }
        return $users;
    }
        ...
        if (null === $this->_getData('user_' . $userId)) {
            ...
            try {
                ...
                if (!array_key_exists(self::PARAM_ID, $user) || !array_key_exists(self::PARAM_EMAIL, $user)) {
                    throw new Effy_Freshdesk_Exception('Wrong user data');
                }
                ...
                if (Mage::app()->useCache(self::CACHE_TYPE)) {
                    ...
                    Mage::app()->saveCache($userJsonCached, $this->getCacheId() . '_' . self::CACHE_ID_SUFFIX_USER, $this->getCacheTags(),Mage::getStoreConfig('effy_freshdesk/cache/expiry_lifetime'));
                    ...
                    Mage::app()->saveCache($userJsonCached, $this->getCacheId() . '_' . self::CACHE_ID_SUFFIX_USER_BY_EMAIL, $this->getCacheTags(), Mage::getStoreConfig('effy_freshdesk/cache/expiry_lifetime'));
                }
...
#                Mage::getSingleton('core/session')->addError($e->getMessage());
    ...
    {
        Mage::app()->removeCache(self::CACHE_ID . '_' . self::CACHE_ID_SUFFIX_USER);
        Mage::app()->removeCache(self::CACHE_ID . '_' . self::CACHE_ID_SUFFIX_AGENTS);
        Mage::app()->removeCache(self::CACHE_ID . '_' . self::CACHE_ID_SUFFIX_USER_BY_EMAIL);
    }
}