...
/**
 * Class Effy_Freshdesk_TicketController
 */
class Effy_Freshdesk_TicketController extends Mage_Core_Controller_Front_Action
{

    /**
     * Access based action allowance
     */
    ...
    public function saveAction()
    {
        try {
            ...
            if (array_key_exists(Effy_Freshdesk_Model_Field::FIELD_DISPLAY_ID, $post)) {
                unset($post[Effy_Freshdesk_Model_Field::FIELD_DISPLAY_ID]);
            }

            $fields = Mage::getModel('effy_freshdesk/field')->getCollection();
            ...
            /*if($this->_getFreshdeskUser()->getId()) {
                $post[Effy_Freshdesk_Model_Field::FIELD_REQUESTER_ID] = $this->_getFreshdeskUser()->getId();
            }*/
            $post[Effy_Freshdesk_Model_Field::FIELD_EMAIL]          = $this->_getCustomer()->getEmail();
            $post[Effy_Freshdesk_Model_Field::FIELD_REQUESTER_NAME] = $this->_getCustomer()->getName();
            $post[Effy_Freshdesk_Model_Field::FIELD_STATUS]         = Mage::helper('freshdesk')->getStatusDefault();
            ...
            if (!$back) {
                $lastTicket = Mage::getModel('effy_freshdesk/ticket')->getCollection()
                    ->setRequester($this->_getCustomer()->getEmail())
                    ->setOrder(Effy_Freshdesk_Model_Field::FIELD_DISPLAY_ID, Varien_Data_Collection::SORT_ORDER_DESC)
                    ->load()
                    ->getFirstItem();
    ...
    public function replyAction()
    {
        ...
        try {
            ...
            Mage::getModel('effy_freshdesk/note')
    ...
    protected function _loadValidTicket($ticketId = null)
    {
        ...
        try {
            /** @var Effy_Freshdesk_Model_Ticket $ticket */
            $ticket = Mage::getModel('effy_freshdesk/ticket')
                ->setRequester($this->_getCustomer()->getEmail())
                ->load($ticketId);
            if ($ticketId = $ticket->getId()) {
                $id = $this->_getFreshdeskUser()->getId();
                if (!$id || $ticket->getRequesterId() != $id) {
                    $collTicket = Mage::getModel('effy_freshdesk/ticket')
                        ->getCollection()
                        ->setRequester($this->_getCustomer()->getEmail())
    ...
    {
        Effy_Freshdesk_Model_Freshdesk_Tickets::cleanCache($this->_getCustomer()->getEmail());
